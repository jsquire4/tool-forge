...
export function checkAdapter(evalCase, runMeta) {
  ...
  // responseContainsAny → stored as _responseContainsAny for the adapter consumer.
  // In eval-runner the field is an array of *groups* (string[][]) — each group must
  // have at least one member present in the response text.  RunChecksInput has no
  // native anyOf check, so callers should handle this via checkResponseContainsAnyGroups()
  // or by composing multiple graders with compose.any().
  if (expect.responseContainsAny !== undefined) {
    // Normalise: if caller passed a flat string[] treat it as a single group
    const raw = expect.responseContainsAny;
    const isGrouped = Array.isArray(raw) && raw.length > 0 && Array.isArray(raw[0]);
    input._responseContainsAny = isGrouped
      ? raw
  ...
  if (expect.schemaData !== undefined) {
    ...
    if (expect.requiredKeys) input.requiredKeys = expect.requiredKeys;
