...
describe('checkAdapter', () => {
  ...
  it('sets schemaData without requiredKeys when typeChecks-only', () => {
    // requiredKeys defaults to [] when schemaData is present but no requiredKeys specified,
    // allowing typeChecks-only validation to run through the gate in run-checks.js
    const input = checkAdapter(
    ...
    expect(input.schemaData).toEqual({ a: 1 });
    expect(input.typeChecks).toEqual({ a: 'number' });
    expect(input.requiredKeys).toEqual([]);
  });
