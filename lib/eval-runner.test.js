...
// pg mock — used by Postgres path tests
const { mockInsertEvalRun, mockInsertEvalRunCases, mockPoolEnd, mockPoolQuery } = vi.hoisted(() => ({
  mockInsertEvalRun: vi.fn().mockResolvedValue(42),
  mockInsertEvalRunCases: vi.fn().mockResolvedValue(undefined),
...
vi.mock('./postgres-store.js', async (importOriginal) => {
  const actual = await importOriginal();
  return {
    ...actual,
    PostgresEvalStore: vi.fn().mockImplementation(() => ({
      insertEvalRun: mockInsertEvalRun,
      insertEvalRunCases: mockInsertEvalRunCases
    }))
  };
});
...
// ── runEvals — Postgres path ──────────────────────────────────────────────

describe('runEvals — Postgres path (DATABASE_URL set)', () => {
  const originalDatabaseUrl = process.env.DATABASE_URL;

  // Helper: create a minimal project root with tool + eval files and a mock llmTurn response
  function setupMinimalProject() {
    const root = mkdtempSync(join(tmpdir(), 'forge-pg-test-'));
    const toolsDir = join(root, 'example', 'tools');
    mkdirSync(toolsDir, { recursive: true });
    const evalsDir = join(root, 'docs', 'examples');
    mkdirSync(evalsDir, { recursive: true });

    writeFileSync(join(root, '.env'), 'ANTHROPIC_API_KEY=test-key-pg\n');
    writeFileSync(join(toolsDir, 'get_weather.tool.js'), `
      export default {
        name: 'get_weather',
  ...
  beforeEach(() => {
    ...
    mockInsertEvalRunCases.mockClear();
  ...
  it('calls insertEvalRun on Postgres when DATABASE_URL is set', async () => {
    const { root, toolsDir, evalsDir } = setupMinimalProject();

    await runEvals('get_weather', { project: { toolsDir, evalsDir } }, root, () => {});

    expect(mockInsertEvalRun).toHaveBeenCalledWith(
      expect.objectContaining({ tool_name: expect.any(String) })
    );
  });

  it('calls pool.end() even when insertEvalRun throws', async () => {
    const { root, toolsDir, evalsDir } = setupMinimalProject();
    mockInsertEvalRun.mockRejectedValueOnce(new Error('DB error'));

    // Should NOT throw — DB failure is non-fatal
    await expect(
      runEvals('get_weather', { project: { toolsDir, evalsDir } }, root, () => {})
    ).resolves.not.toThrow();

    expect(mockPoolEnd).toHaveBeenCalled();
  });

  it('runEvals succeeds and returns results even when DB write fails', async () => {
    const { root, toolsDir, evalsDir } = setupMinimalProject();
    mockInsertEvalRun.mockRejectedValueOnce(new Error('connection refused'));

    const stderrSpy = vi.spyOn(process.stderr, 'write').mockImplementation(() => {});

    const result = await runEvals('get_weather', { project: { toolsDir, evalsDir } }, root, () => {});

    // Eval ran successfully; DB failure is non-fatal
    expect(result).toMatchObject({ total: expect.any(Number) });
    expect(result.passed + result.failed + result.skipped).toBe(result.total);

    // DB error must be reported to stderr with the expected prefix
    expect(stderrSpy).toHaveBeenCalledWith(
      expect.stringContaining('[eval-runner] DB write failed (non-fatal):')
    );
    stderrSpy.mockRestore();
  });

  it('does NOT call pool.end() when pool is injected (ownPool=false)', async () => {
    // When runEvals() receives an injected pool it must not drain it —
    // the caller (runEvalsMultiPass) owns the pool lifecycle.
    const { root, toolsDir, evalsDir } = setupMinimalProject();

    // Create an injected pool object with its own end spy (distinct from mockPoolEnd
    // which belongs to the auto-constructed Pool used by the "own pool" path).
    const injectedEndSpy = vi.fn().mockResolvedValue(undefined);
    const injectedPool = {
      query: vi.fn().mockResolvedValue({ rows: [] }),
      end: injectedEndSpy
    };

    // PostgresEvalStore is already mocked at module level to use mockInsertEvalRun /
    // mockInsertEvalRunCases, so the store will succeed and only the pool.end() branch
    // is what we're testing here.
    await runEvals('get_weather', { project: { toolsDir, evalsDir } }, root, () => {}, { pgPool: injectedPool });

    // pool.end() must NOT be called on the injected pool
    expect(injectedEndSpy).not.toHaveBeenCalled();
  });

  it('does NOT create Postgres pool when DATABASE_URL is unset', async () => {
    delete process.env.DATABASE_URL;

    const { Pool: PoolMock } = (await import('pg')).default;
    PoolMock.mockClear?.();

    // Need a fresh LLM turn mock since beforeEach reset it
    const root = mkdtempSync(join(tmpdir(), 'forge-pg-nourl-'));
    const toolsDir = join(root, 'example', 'tools');
    mkdirSync(toolsDir, { recursive: true });
    const evalsDir = join(root, 'docs', 'examples');
    mkdirSync(evalsDir, { recursive: true });

    writeFileSync(join(root, '.env'), 'ANTHROPIC_API_KEY=test-key-nourl\n');
    writeFileSync(join(toolsDir, 'get_weather.tool.js'), `
      export default {
        name: 'get_weather',
        description: 'Get current weather',
    ...
    // Pool constructor should not have been called — SQLite path was used instead
