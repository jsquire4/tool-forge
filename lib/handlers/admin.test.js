...
describe('Admin API', () => {
  ...
  describe('overlay persistence', () => {
    ...
    it('persists conversation section to forge.config.json', async () => {
      fsMocked.readFileSync.mockReturnValue(JSON.stringify({ conversation: { window: 20 } }));
      const ctx = {
        ...makeCtx(),
        configPath: '/fake/forge.config.json'
      };
      const res = makeRes();
      await handleAdminConfig(
        makeReq('PUT', '/forge-admin/config/conversation', { window: 50 }, 'admin-secret'),
        res, ctx
      );
      const written = fsMocked.writeFileSync.mock.calls[0][1];
      expect(written).toContain('"window"');
      expect(fsMocked.renameSync).toHaveBeenCalled();
    });
  });
  ...
  it('returns 405 for unsupported methods', async () => {
    const res = makeRes();
    await handleAdminConfig(
      makeReq('DELETE', '/forge-admin/config/model', null, 'admin-secret'),
      res, makeCtx()
    );
