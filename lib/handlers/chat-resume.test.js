...
describe('handleChatResume', () => {
  ...
  describe('handleChatResume — chatAuditStore path', () => {
    ...
    it('does not throw when chatAuditStore is null and db handles audit', async () => {
      const token = makeJwt({ sub: 'fallback-resume-user' });
      const ctx = makeCtx(db);
      ...
      await expect(
        handleChatResume(makeReq({ resumeToken, confirmed: true }, token), res, ctx)
