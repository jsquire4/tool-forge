...
describe('handleChatSync', () => {
  ...
  describe('rate limiting', () => {
    it('returns 429 when rate limiter blocks', async () => {
      ...
      const row = db.prepare('SELECT * FROM chat_audit ORDER BY id DESC LIMIT 1').get();
      expect(row).toBeDefined();
      expect(row.status_code).toBe(429);
    });
  });

  describe('handleChatSync — chatAuditStore path', () => {
    it('calls chatAuditStore.insertChatAudit instead of sqlite when chatAuditStore is set', async () => {
      const token = makeJwt({ sub: 'pg-sync-user' });
    ...
    it('does not throw when chatAuditStore is null and db handles audit', async () => {
      const token = makeJwt({ sub: 'fallback-sync-user' });
      const ctx = makeCtx(db);
      ...
      await expect(handleChatSync(makeReq({ message: 'hello' }, token), res, ctx)).resolves.toBeUndefined();
      expect(res.writeHead).toHaveBeenCalledWith(200, expect.any(Object));
    });
  });
});
