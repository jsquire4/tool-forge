...
describe('handleChat', () => {
  ...
  describe('audit logging', () => {
    it('inserts a chat_audit row after a successful request', async () => {
      const token = makeJwt({ sub: 'user-audit' });
      const res = makeRes();
      ...
      await handleChat(makeReq({ message: 'audit test' }, token), res, makeCtx(db));

      const rows = db.prepare("SELECT * FROM chat_audit WHERE route = '/agent-api/chat'").all();
      expect(rows.length).toBeGreaterThanOrEqual(1);
      const row = rows[rows.length - 1];
      expect(row.user_id).toBe('user-audit');
      expect(row.route).toBe('/agent-api/chat');
  ...
  describe('rate limiting', () => {
    ...
    it('passes through when rate limiter allows', async () => {
      ...
      expect(ctx.rateLimiter.check).toHaveBeenCalledWith('rl-user', expect.any(String));
    });
  });

  describe('handleChat — chatAuditStore path', () => {
    it('calls chatAuditStore.insertChatAudit instead of sqlite when chatAuditStore is set', async () => {
      const token = makeJwt({ sub: 'pg-user' });
      const insertChatAudit = vi.fn().mockResolvedValue(1);
      const ctx = makeCtx(db);
      ctx.chatAuditStore = { insertChatAudit };
      ...
      const { req, res } = { req: makeReq({ message: 'hello' }, token), res: makeRes() };
      await handleChat(req, res, ctx);

      expect(insertChatAudit).toHaveBeenCalledWith(
    ...
    it('does not throw when chatAuditStore is null and db handles audit', async () => {
      const token = makeJwt({ sub: 'fallback-user' });
      const ctx = makeCtx(db);
      ctx.chatAuditStore = null;
      ...
      const req = makeReq({ message: 'hello' }, token);
      const res = makeRes();
      // Should complete without throwing — db fallback handles audit
      await expect(handleChat(req, res, ctx)).resolves.toBeUndefined();
