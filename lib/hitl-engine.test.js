...
describe('HitlEngine', () => {
  ...
  it('makeHitlEngine factory uses pgPool when provided (no redis)', async () => {
    const pgMock = {
      ...
      async query(sql, params) {
        ...
        if (sql.includes('DELETE') && sql.includes('RETURNING')) { const r = this._rows.get(params[0]) ?? null; this._rows.delete(params[0]); return { rows: r ? [r] : [] }; }
  ...
  it('makeHitlEngine factory prefers redis when provided', async () => {
    ...
    const redisMock = {
      ...
      async getDel(k) { const v = this.store.get(k) ?? null; this.store.delete(k); return v; },
  ...
  describe('SQLite cleanup interval', () => {
    ...
    it('does NOT create cleanup interval for Redis backend', () => {
      const redisMock = {
        async set() {}, async get() { return null; }, async getDel() { return null; }, async del() {}
      };
  ...
  describe('destroy()', () => {
    it('clears in-memory cleanup timer', () => {
      vi.useFakeTimers();
      const engine = new HitlEngine(); // creates _cleanupTimer
      expect(engine._cleanupTimer).toBeDefined();
      engine.destroy();
      expect(engine._cleanupTimer).toBeNull();
      vi.useRealTimers();
    });

    it('clears SQLite cleanup timer', () => {
      vi.useFakeTimers();
      const db = makeTestDb();
      const engine = new HitlEngine({ db }); // creates _sqliteCleanupTimer
      expect(engine._sqliteCleanupTimer).toBeDefined();
      engine.destroy();
      expect(engine._sqliteCleanupTimer).toBeNull();
      vi.useRealTimers();
    });

    it('clears Postgres cleanup timer after table creation', async () => {
      const pgPool = {
        _rows: new Map(),
        async query(sql, params) {
          if (sql.includes('CREATE TABLE')) return { rows: [] };
      ...
      const engine = new HitlEngine({ pgPool });
      // Trigger table creation (which sets up the pg cleanup timer)
      await engine.pause({ test: true });
      expect(engine._pgCleanupTimer).toBeDefined();
      engine.destroy();
      expect(engine._pgCleanupTimer).toBeNull();
    });

    it('is safe to call before Postgres table is created (no timer yet)', () => {
      const pgPool = {
        async query() { return { rows: [] }; }
      };
      const engine = new HitlEngine({ pgPool });
      // _pgCleanupTimer is undefined at this point (table not created yet)
