...
describe('RateLimiter', () => {
  ...
  describe('Redis backend', () => {
    function createRedisMock() {
      ...
      return {
        store,
        async set(key, value, flag, ttl, nx) {
          if (nx === 'NX') {
            if (store.has(key)) return null;
            store.set(key, value);
            // Track the EX ttl so the "sets TTL" test can find it
            if (flag === 'EX' && ttl !== undefined) store.set(key + ':ttl', ttl);
            return 'OK';
          }
          store.set(key, value);
          return 'OK';
  ...
  describe('makeRateLimiter factory', () => {
    ...
    it('passes redis client to limiter', async () => {
      ...
      const redis = {
        store,
        async set(k, v, flag, ttl, nx) { if (nx === 'NX') { if (store.has(k)) return null; store.set(k, v); return 'OK'; } store.set(k, v); return 'OK'; },
