...
import { writeFixture, readFixture } from '../fixtures/fixture-store.js';
...
export async function runEvalSuite(evalFilePath, agentFn, opts = {}) {
  ...
  for (const evalCase of cases) {
    ...
    if (replay) {
      ...
      const configHash = JSON.stringify(evalCase.expect ?? {});
    ...
    if (record) {
      const caseId = evalCase.id ?? message.slice(0, 40);
      const configHash = JSON.stringify(evalCase.expect ?? {});
      await writeFixture(fixturesDir, caseId, configHash, { responseText, toolsCalled }).catch(() => {});
    }
...
function checkCase(evalCase, { responseText, toolsCalled, latencyMs, cost }) {
  ...
  if (expect.responseContainsAny?.length && responseText.length > 0) {
    const anyResult = checkResponseContainsAnyGroups(responseText, expect.responseContainsAny);
    if (!anyResult.pass) failures.push(...anyResult.failures);
  }
