...
export class VerifierRunner {
   ...
   * @param {import('pg').Pool|null} [pgPool] — optional Postgres pool (takes priority over SQLite)
   * @param {import('./verifier-worker-pool.js').VerifierWorkerPool} [workerPool] — optional pre-created pool
   */
  constructor(db, config = {}, pgPool = null, workerPool = null) {
    this._db = db;
    this._config = config;
    this._pgPool = pgPool;
  ...
  async loadFromDb(db) {
    ...
    if (!targetDb && !this._pgPool) return;
    ...
    let allVerifiers, allBindings;
    if (this._pgPool) {
      const { rows: v } = await this._pgPool.query(
        `SELECT * FROM verifier_registry WHERE enabled = 1 ORDER BY aciru_order ASC`);
      const { rows: b } = await this._pgPool.query(
        `SELECT * FROM verifier_tool_bindings WHERE enabled = 1`);
      allVerifiers = v;
      allBindings  = b;
    } else {
      allVerifiers = targetDb.prepare(
        'SELECT * FROM verifier_registry WHERE enabled = 1 ORDER BY aciru_order ASC'
      ).all();
      allBindings = targetDb.prepare(
        'SELECT * FROM verifier_tool_bindings WHERE enabled = 1'
      ).all();
    }
    ...
    for (const [key, verifiers] of toolMap) {
      verifiers.sort((a, b) => (a.order ?? 'Z-9999').localeCompare(b.order ?? 'Z-9999'));
  ...
  logResult(sessionId, toolName, result) {
    if (this._pgPool) {
      // Fire-and-forget Postgres write (non-fatal, same contract as SQLite path)
      this._pgPool.query(
        `INSERT INTO verifier_results (session_id, tool_name, verifier_name, outcome, message, created_at)
         VALUES ($1,$2,$3,$4,$5,$6)`,
        [sessionId, toolName, result.verifierName ?? 'unknown', result.outcome,
         result.message ?? null, new Date().toISOString()]
