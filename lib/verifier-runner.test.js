...
describe('VerifierRunner', () => {
  ...
  describe('VerifierRunner — pgPool path', () => {
    it('stores pgPool as third constructor argument', () => {
      const db = { prepare: vi.fn() };
      const mockPool = { query: vi.fn() };
      const runner = new VerifierRunner(db, {}, mockPool);
      expect(runner._pgPool).toBe(mockPool);
    });

    it('workerPool is still accessible as fourth argument', () => {
      const mockPool = { query: vi.fn() };
      const mockWorkerPool = { dispatch: vi.fn() };
      const runner = new VerifierRunner(null, {}, mockPool, mockWorkerPool);
      expect(runner._pgPool).toBe(mockPool);
      expect(runner._workerPool).toBe(mockWorkerPool);
    });

    it('loadFromDb returns early when both db and pgPool are null', async () => {
      const runner = new VerifierRunner(null, {}, null);
      // Should not throw
      await expect(runner.loadFromDb(null)).resolves.toBeUndefined();
    });

    it('loadFromDb queries pgPool when pgPool is set', async () => {
      const mockVerifiers = [];
      const mockBindings = [];
      const mockPool = {
        query: vi.fn()
          .mockResolvedValueOnce({ rows: mockVerifiers })  // verifier_registry query
          .mockResolvedValueOnce({ rows: mockBindings })   // verifier_tool_bindings query
      };
      const runner = new VerifierRunner(null, { verification: { sandbox: false } }, mockPool);
      await runner.loadFromDb(null);
      expect(mockPool.query).toHaveBeenCalledTimes(2);
      // First call should query verifier_registry
      expect(mockPool.query.mock.calls[0][0]).toContain('verifier_registry');
      // Second call should query verifier_tool_bindings
      expect(mockPool.query.mock.calls[1][0]).toContain('verifier_tool_bindings');
    });

    it('loadFromDb uses pgPool (not sqlite) when both are provided', async () => {
      const mockPool = {
        query: vi.fn()
          .mockResolvedValueOnce({ rows: [] })
          .mockResolvedValueOnce({ rows: [] })
      };
      const sqliteDb = { prepare: vi.fn() };
      const runner = new VerifierRunner(sqliteDb, { verification: { sandbox: false } }, mockPool);
      await runner.loadFromDb(sqliteDb);
      // pgPool.query should be called, not sqliteDb.prepare
      expect(mockPool.query).toHaveBeenCalled();
      expect(sqliteDb.prepare).not.toHaveBeenCalled();
    });
  });
});
